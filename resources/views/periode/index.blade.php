    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="bg-[#f9fafb] rounded-lg p-6 shadow-md">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('periode.create') }}" class="bg-blue-600 text-white py-2 px-6 rounded-lg shadow-md hover:bg-blue-700">
                    + Tambah Periode
                </a>
            </div>
                ...
                <table class="min-w-full table-auto text-sm text-gray-900 border-collapse rounded-lg">
                    <thead class="bg-[#e2e8f0] text-left">
                        <tr>
                            ...
                            <th class="px-6 py-3 w-1/6 border-b">Tanggal Mulai</th>
                            <th class="px-6 py-3 w-1/6 border-b">Tanggal Berakhir</th>
                    ...
                    <tbody>
                        @foreach ($periodes as $index => $periode)
                            <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100 transition-all duration-200">
                                ...
                                <!-- Menampilkan Nama Siswa (Bisa lebih dari satu) -->
                                <td class="px-6 py-4 border-b">
                                    {{ implode(', ', $periode->students->pluck('name')->toArray()) }}
                                </td>

                                <!-- Menampilkan Nama DUDI -->
                                <td class="px-6 py-4 border-b">{{ $periode->dudi->namadudi }}</td>
